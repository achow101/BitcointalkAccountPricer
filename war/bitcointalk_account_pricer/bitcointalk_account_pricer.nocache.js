...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "bitcointalk_account_pricer" because\n';
  ...
  $wnd.__gwt_activeModules['bitcointalk_account_pricer'] = {
    'moduleName' : 'bitcointalk_account_pricer',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/bitcointalk_account_pricer.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['bitcointalk_account_pricer'].superdevmode = true;

  var expectedSuffix = '/bitcointalk_account_pricer.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + bitcointalk_account_pricer + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:bitcointalk_account_pricer';
  var devModeSessionKey = '__gwtDevModeSession:bitcointalk_account_pricer';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/bitcointalk_account_pricer/bitcointalk_account_pricer.nocache.js';
  ...
  $wnd.__gwt_activeModules['bitcointalk_account_pricer'].canRedirect = true;
  $wnd.__gwt_activeModules['bitcointalk_account_pricer'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING bitcointalk_account_pricer';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/bitcointalk_account_pricer?user.agent=' + ua + '&_callback=' + callback;
